...
/// Runtime settings cannot enable cloud trace uploads in this fork.
#[tokio::test(flavor = "current_thread")]
async fn build_harness_trace_uploads_stays_empty_with_upload_config_enabled() {
    let agent = build_minimal_agent_for_tests();
    enable_trace_upload_config(&agent);
    agent.cfg.borrow_mut().endpoints.trace_upload_bucket =
        Some("gs://harness-trace-test".to_string());
...
#[tokio::test(flavor = "current_thread")]
async fn upload_harness_trace_turns_disabled_policy_preserves_counter() {
    let agent = build_minimal_agent_for_tests();
    enable_trace_upload_config(&agent);
    agent.cfg.borrow_mut().endpoints.trace_upload_bucket =
        Some("gs://harness-trace-test".to_string());
...
#[tokio::test]
#[serial_test::serial]
async fn feedback_trace_offer_suppressed_for_personal_oauth_accounts() {
    ...
    assert!(
        agent
            ...
            .await
            .is_none(),
        "this fork never offers a remote trace upload"
    );
}
#[tokio::test]
#[serial_test::serial]
...
/// Request uploads through config to verify the fork's disabled policy wins.
...
#[tokio::test]
async fn product_analytics_disabled_even_with_telemetry_on() {
    let agent = build_agent_with_auth(crate::auth::GrokAuth::test_default());
...
#[tokio::test]
async fn diagnostic_upload_unavailable_for_all_auth_states() {
    let agents = [
        build_minimal_agent_for_tests(),
        build_agent_with_auth(crate::auth::GrokAuth::test_default()),
        build_agent_with_auth(crate::auth::GrokAuth {
            coding_data_retention_opt_out: true,
            ..crate::auth::GrokAuth::test_default()
        }),
    ];
...
#[tokio::test]
async fn collection_config_gate_stays_disabled_across_trace_upload_flip() {
    let agent = build_agent_with_auth(crate::auth::GrokAuth::test_default());
    enable_trace_upload_config(&agent);
    agent.sync_collection_config_gate();
    assert!(
        ...
        "config cannot enable remote trace upload"
    );
    {
        let mut cfg = agent.cfg.borrow_mut();
        cfg.features.telemetry = Some(crate::agent::config::TelemetryMode::Disabled);
